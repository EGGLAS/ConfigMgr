...
if (-not $TierOUs) {
    Log -Message "No Tier OUs found under $AdminRootOU — nothing to do." -Type 2 -Component "Script" -LogFile $LogFile
    exit 0
}
...
foreach ($TierOU in $TierOUs) {
    ...
    foreach ($SubOU in $SubOUs) {
        ...
        foreach ($Computer in $Computers) {
            ...
            $Description = "Local admin group for $($Computer.Name) - $TierPrefix $TypePrefix (Auto-created)"
